<template>
  ...
  <form @submit.prevent="handleAddStudents" class="md:w-[50%] sm:w-full  p-6 shadow-xl">
    ...
    <div class="mb-4 ">
      ...
      <div class="mb-4 ">
        ...
        <select class="w-full   py-4 rounded" required v-model="sclassroom" id="">
          <option disabled selected hidden value="">ClassRoom</option>
          <option v-for="cl of classrooms" :value="cl.id">{{cl.classroom_name}}</option>
        </select>
      </div>
<!--      <div class="mb-4 ">-->
<!--        <label>Classroom</label>-->
<!--        <select class="w-full py-4 rounded" v-model="sclassroom" required >-->
<!--          <option disabled selected hidden value="">classroom</option>-->
<!--          <option v-for="c of classrooms" :value="c.id">{{c.classroom_name}}</option>-->
<!--        </select>-->
<!--      </div>-->
      <div class="mb-4 flex-1">
        <input class="w-full py-4 rounded" required type="datetime-local" v-model="sdob" placeholder="Date Of Birth">
      </div>
    </div>
    <hr>
...
import {useClassRoomStore} from "~/store/classrooms/classrooms";
...
const sdob = ref("")
const sparentId = ref("")
const sclassroom = ref("")
const parents = reactive([])
const classStore = useClassRoomStore()
...
async function handleAddStudents(){
  if(sf_name.value.trim() === "" || sl_name.value.trim() === "" || sl_name.value.trim() ===""  || sclassroom.value === ""){
  ...
  // }
  //
  const response = await axios.post("/api/students/store" , studentBody , {method:"POST" , headers:{'Content-Type':'Application/json'}})
  if(!response){
